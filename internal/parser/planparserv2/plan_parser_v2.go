...
func handleExpr(schema *typeutil.SchemaHelper, exprStr string, rootCoord types.RootCoord) interface{} {
	if exprStr == "" {
		return nil
	}
	...
	visitor := NewParserVisitor(schema, rootCoord)
	return ast.Accept(visitor)
}

func ParseExpr(schema *typeutil.SchemaHelper, exprStr string, rootCoord types.RootCoord) (*planpb.Expr, error) {
	if len(exprStr) <= 0 {
		return nil, nil
	}

	ret := handleExpr(schema, exprStr, rootCoord)
...
func CreateRetrievePlan(schemaPb *schemapb.CollectionSchema, exprStr string, rootCoord types.RootCoord) (*planpb.PlanNode, error) {
	schema, err := typeutil.CreateSchemaHelper(schemaPb)
	if err != nil {
		return nil, err
	}

	expr, err := ParseExpr(schema, exprStr, rootCoord)
	if err != nil {
		return nil, err
	}
...
func CreateSearchPlan(schemaPb *schemapb.CollectionSchema, exprStr string, vectorFieldName string, queryInfo *planpb.QueryInfo, rootCoord types.RootCoord) (*planpb.PlanNode, error) {
	schema, err := typeutil.CreateSchemaHelper(schemaPb)
	if err != nil {
		return nil, err
	}
